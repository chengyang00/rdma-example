...
{
	return memcmp((void *)src, (void *)dst, str_len);
}
...
/* send file to server */
static int send_file_to_server()
{
	int ret = -1;
	/* sync with server */
...
{
	printf("Usage:\n");
	printf("rdma_client: [-a <server_addr>] [-p <server_port>] -f <file name> (required)\n");
...
int main(int argc, char **argv)
{
	...
	while ((option = getopt(argc, argv, "s:a:p:f:")) != -1)
	{
		switch (option)
		{
		case 'a':
		...
		case 'f':
			/* file name */
			printf("Passed file name is %s\n", optarg);
			FILE *file = fopen(optarg, "r");
			if (file == NULL)
			{
				rdma_error("Failed to open file : -%d\n", errno);
				return -errno;
			}
			...
			str_len = file_size + 1;
			rewind(file);
			src = (char *)malloc(str_len * sizeof(char));
			if (!src)
			{
				rdma_error("Failed to allocate memory : -ENOMEM\n");
				return -ENOMEM;
			}
			size_t result = fread(src, sizeof(char), file_size, file);
			src[file_size] = 0;
			fclose(file);
			if (result != file_size)
			{
				rdma_error("Failed to read file : -%d\n", errno);
				return -errno;
			}
			dst = (char *)malloc(str_len * sizeof(char));
			if (!dst)
			{
