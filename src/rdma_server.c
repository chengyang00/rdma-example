...
static int recv_file_from_client()
{
	int ret = -1;
	int str_len = client_metadata_attr.length;
	content = (char *)malloc(str_len);
	...
	/* if all good, then we receive a file */
	FILE *file = fopen("output", "w");
	size_t result = fwrite(content, sizeof(char), str_len - 1, file);
	fclose(file);
	if (result != str_len - 1)
	{
		rdma_error("Failed to write file : -%d\n", errno);
		return -errno;
	}
	return 0;
}
